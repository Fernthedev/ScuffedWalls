...
public class ScuffedFunction
{
    ...
    public void InstantiateSFunction(FunctionRequest request, Workspace instance, float time, int repeat)
    {
        ...
        Stats = new MapStats();
        RepeatCount = repeat;
        if (UnderlyingParameters != null)
            foreach (var param in UnderlyingParameters)
                param.Variables.Register(Variables);
        Init();
    }
    ...
    protected void AddRefresh(string file)
    {
        if (GetParam("refreshonsave", false, p => bool.Parse(p)))
            ScuffedWallsContainer.FilesToChange.Add(
                new FileChangeDetector(new FileInfo(file)));
    }
    ...
    {
        var result = UnderlyingParameters.Get(Name.ToLower());
        if (result != null)
        {
            result.WasUsed = true;
