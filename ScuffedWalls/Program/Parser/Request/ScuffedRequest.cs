...
public class ScuffedRequest : Request
{
    ...
    public List<ContainerRequest> WorkspaceRequests { get; } = new();
    public List<ContainerRequest> CustomFunctionRequests { get; } = new();

    public ContainerRequest GetCustomFunction(string name)
    {
        return CustomFunctionRequests.FirstOrDefault(p =>
            p.Name.ToLower().RemoveWhiteSpace() == name.ToLower().RemoveWhiteSpace());
    }

    public bool CustomFunctionExists(string name)
    {
        return CustomFunctionRequests.Any(p =>
            p.Name.ToLower().RemoveWhiteSpace() == name.ToLower().RemoveWhiteSpace());
    }
    ...
    public override Request SetupFromLines(List<Parameter> Lines)
    {
        ...
        var includes = new List<FileInfo>();


        var previousType = Type.ScuffedRequest;
        ...
        foreach (var include in includes)
        {
            ...
            var requestToInclude =
                (ScuffedRequest)new ScuffedRequest().SetupFromLines(new ScuffedWallFile(include.FullName).Parameters);
        ...
        void addWspc()
        {
            if (_paramScanner.Current == null ||
                _paramScanner.Current.Clean.Name == ContainerRequest.WorkspaceKeyword ||
                _paramScanner.Current.Clean.Name == ContainerRequest.DefineKeyword)
            {
                switch (previousType)
                {
                    case Type.WorkspaceRequest:
                        WorkspaceRequests.Add(
                            (ContainerRequest)new ContainerRequest().SetupFromLines(_paramScanner.GetAndResetCache()));
                        break;
                    case Type.DefineRequest:
                        var def = (ContainerRequest)new ContainerRequest().SetupFromLines(
                            _paramScanner.GetAndResetCache());
                        CustomFunctionRequests.Add(def);

                        var param = string.Join(", ", def.VariableRequests.Where(v => v.Public).Select(v => v.Name));

                        ScuffedWalls.Print(
                            $"Registered Custom Function \"{def.Name}\" {(string.IsNullOrEmpty(param) ? "" : $"Params: {{{param}}}")}");
                        break;
        ...
        void addDefaults()
        {
            ...
            foreach (var line in cache)
                ...
                {
                    case "include":
                        includes.Add(new FileInfo(Path.Combine(ScuffedWallsContainer.ScuffedConfig.MapFolderPath,
                            line.StringData.Trim())));
                        break;
                    case "hidemapinrpc":
