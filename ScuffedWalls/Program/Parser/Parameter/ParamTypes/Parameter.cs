...
public class Parameter : INameStringDataPair, ICloneable
{
    ...
    public StringComputationExcecuter Computer { get; }
    public TreeList<AssignableInlineVariable> Variables => Computer.Variables;
    public Variable Raw { get; } = new();

    public Variable Clean =>
        new(Raw?.Name?.ToLower().RemoveWhiteSpace(), Raw?.StringData?.ToLower().RemoveWhiteSpace());
    ...
    public static void Check(IEnumerable<Parameter> parameters)
    {
        ...
        {
            ScuffedWalls.Print($"{(isFirst ? "↑" : "|")} Unused Parameter \"{p.Name.Trim()}\" at line {p.GlobalIndex}",
                ScuffedWalls.LogSeverity.Warning, ShowStackFrame: false);
            isFirst = false;
        }
        // ScuffedWalls.Print($" ---------------------------------------------------------------------------------------------", ScuffedWalls.LogSeverity.Warning, ShowStackFrame: false);
    }
