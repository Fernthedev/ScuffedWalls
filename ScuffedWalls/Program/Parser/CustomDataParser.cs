...
{
    public static readonly CustomDataParser Instance = new();
    ...
    public static Func<string, object> NestedArrayDefaultStringConverter =>
        val => DeserializeDefaultToString<object[][]>($"[{val}]");

    public ICustomDataMapObject ReadToCustomData(TreeList<Parameter> parameters, ICustomDataMapObject instance)
    {
        this.parameters = parameters;
        instance._customData = Read(out var time);
    ...
    public TreeDictionary Read(out float? time)
    {
        ...
        var customdata = new TreeDictionary
        {
            ...
            ["_rotation"] = GetParam("rotation", null, p => JsonSerializer.Deserialize<object[]>(p)) ??
                            GetParam("Crotation", null, p => (object)float.Parse(p)),
    ...
    private TreeDictionary getAnimation(bool isEventData)
    {
        ...
        {
            ["_definitePosition"] = GetParam("animatedefiniteposition", null,
                p => DeserializeDefaultToString<object[][]>($"[{p}]")),
            ...
            ["_dissolveArrow"] = GetParam("animatedissolvearrow", null,
                p => DeserializeDefaultToString<object[][]>($"[{p}]")),
            ...
            ["_localRotation"] = GetParam("animatelocalrotation", null,
                p => DeserializeDefaultToString<object[][]>($"[{p}]")),
            ["_scale"] = GetParam("animatescale", null, p => DeserializeDefaultToString<object[][]>($"[{p}]")),
            ["_interactable"] = GetParam("animateinteractable", null,
                p => DeserializeDefaultToString<object[][]>($"[{p}]")),
            ["_time"] = GetParam("animatetime", null, p => DeserializeDefaultToString<object[][]>($"[{p}]")),
            ["_height"] = GetParam("animateheight", null, p => JsonSerializer.Deserialize<object[][]>($"[{p}]")) ??
                          GetParam("defineanimateheight", null, p => (object)p),
            ["_attenuation"] =
                GetParam("animateattenuation", null, p => JsonSerializer.Deserialize<object[][]>($"[{p}]")) ??
                GetParam("defineanimateattenuation", null, p => (object)p),
            ["_startY"] = GetParam("animatestartY", null, p => JsonSerializer.Deserialize<object[][]>($"[{p}]")) ??
                          GetParam("defineanimatestartY", null, p => (object)p),
            ["_offset"] = GetParam("animatoffset", null, p => JsonSerializer.Deserialize<object[][]>($"[{p}]")) ??
                          GetParam("defineanimatoffset", null, p => (object)p)
        };
        if (isEventData)
        {
            ...
            data["_localPosition"] =
                GetParam("animatelocalposition", null, p => JsonSerializer.Deserialize<object[][]>($"[{p}]")) ??
                GetParam("defineanimatelocalposition", null, p => (object)p);
            data["_localScale"] =
                GetParam("animatelocalscale", null, p => JsonSerializer.Deserialize<object[][]>($"[{p}]")) ??
                GetParam("defineanimatelocalscale", null, p => (object)p);
    ...
    private static object DeserializeDefaultToString<T>(string JSON)
    {
        char[] JSONChars = { ',' };
        try
        {
