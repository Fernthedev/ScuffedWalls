...
internal static class ScuffedWalls
{
    ...
    private static readonly ConsoleColor[] sevColor =
    {
        ConsoleColor.Gray,
        ConsoleColor.Cyan,
        ConsoleColor.Yellow,
    ...
    private static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            Print(
                $"Waiting for changes to {string.Join(", ", ScuffedWallsContainer.FilesToChange.Select(file => file.File.Name))}");
    ...
    private static void ExecuteRequest()
    {
        ...
        Debug.TryAction(
            () =>
            {
                Request = (ScuffedRequest)new ScuffedRequest().SetupFromLines(ScuffedWallsContainer.ScuffedWallFile
                    .Parameters);
            },
            e =>
            {
        ...
        Debug.TryAction(() =>
        {
            Print($"Writing to {new FileInfo(ScuffedWallsContainer.ScuffedConfig.MapFilePath).Name}",
                ShowStackFrame: false);
            File.WriteAllText(ScuffedWallsContainer.ScuffedConfig.MapFilePath,
                JsonSerializer.Serialize(Parser.Result,
                    new JsonSerializerOptions
                    {
                        IgnoreNullValues = true, WriteIndented = ScuffedWallsContainer.ScuffedConfig.PrettyPrintJson
                    }));
            ...
            Print(string.Join(' ', Parser.Result.Stats.Select(st => $"[{st.Value} {st.Key.MakePlural(st.Value)}]")),
                ShowStackFrame: false);
        }, e => { Print($"Error saving to map file ERROR: {(e.InnerException ?? e).Message}", LogSeverity.Critical); });
        Print("Saving Config");
        File.WriteAllText(ScuffedWallsContainer.ConfigFileName,
            JsonSerializer.Serialize(ScuffedWallsContainer.ScuffedConfig,
                new JsonSerializerOptions { IgnoreNullValues = true, WriteIndented = true }));
    }

    public static void Print(string Message, LogSeverity Severity = LogSeverity.Info, ConsoleColor? Color = null,
        StackFrame StackFrame = null, bool ShowStackFrame = true, string OverrideStackFrame = null)
    {
        ...
        var stack = methodInfo.DeclaringType.Name.Replace("ScuffedWalls", "Main");
        var message =
            OverrideStackFrame != null ? $"{OverrideStackFrame} - {Message}" :
            ShowStackFrame && Severity < LogSeverity.Error ? $"{stack} - {Message}" :
            Message;
    ...
    private static void printStats()
    {
        ...
        for (var i = 0; i < logSevCount; i++)
            if (debugStats[i] > 0)
                stat.Add($"[{debugStats[i]} {((LogSeverity)i).ToString().MakePlural(debugStats[i])}]");
        Print(string.Join(' ', stat), Color: getHighestSev(), ShowStackFrame: false);
        debugStats = new int[logSevCount];

        ConsoleColor getHighestSev()
        {
            var highest = LogSeverity.Info;
            for (var i = 0; i < logSevCount; i++)
                if (debugStats[i] > 0)
                    highest = (LogSeverity)i;
