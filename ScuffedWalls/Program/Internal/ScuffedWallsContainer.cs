...
";
    ...
    public static void Initialize(string[] argss)
    {
        ...
        try
        {
            InfoDifficulty = Info.At<IEnumerable<object>>("_difficultyBeatmapSets").Cast<TreeDictionary>()
                .Where(set => set.At<IEnumerable<object>>("_difficultyBeatmaps").Cast<TreeDictionary>().Any(dif =>
                    dif["_beatmapFilename"].ToString() == new FileInfo(ScuffedConfig.MapFilePath).Name))
                .First().At<IEnumerable<object>>("_difficultyBeatmaps").Cast<TreeDictionary>()
                .Where(dif => dif["_beatmapFilename"].ToString() == new FileInfo(ScuffedConfig.MapFilePath).Name)
                .First();
        }
        catch (Exception e)
        {
        ...
        BPMAdjuster = new BpmAdjuster(Info["_beatsPerMinute"].ToFloat(),
            InfoDifficulty["_noteJumpMovementSpeed"].ToFloat(), InfoDifficulty["_noteJumpStartBeatOffset"].ToFloat());
        ScuffedWalls.Print(
            $"Njs: {BPMAdjuster.Njs} Offset: {BPMAdjuster.StartBeatOffset} HalfJump: {BPMAdjuster.HalfJumpBeats}");
    ...
    {
        File.Copy(ScuffedConfig.MapFilePath,
            Path.Combine(ScuffedConfig.BackupPaths.BackupMAPFolderPath, $"{DateTime.Now.ToFileString()}.dat"));
    }
    ...
    public static void Check(BeatMap map)
    {
        try
        {
            ...
            if (!(requirements.Any(r => r.ToString() == "Chroma") || suggestions.Any(s => s.ToString() == "Chroma")) &&
                map.NeedsChroma()) requirements.Add("Chroma");
        }
        catch
        {
            //void
    ...
    public static void VerifySW()
    {
        ...
        {
            using (var file = new StreamWriter(ScuffedConfig.SWFilePath))
            {
...
0: Import
   Path:{new FileInfo(ScuffedConfig.OldMapPath).Name}");
    ...
    {
        var client = new GitHubClient(new ProductHeaderValue("ScuffedWalls"));
        ...
        if (latest.TagName != ScuffedWalls.Version)
            ScuffedWalls.Print($"Update Available! Latest Ver: {latest.Name} ({latest.HtmlUrl})",
                ScuffedWalls.LogSeverity.Notice, ShowStackFrame: false);
    }

    public static TreeDictionary GetInfo()
    {
        TreeDictionary info = null;
        if (File.Exists(ScuffedConfig.InfoPath))
        {
            info = JsonSerializer.Deserialize<TreeDictionary>(File.ReadAllText(ScuffedConfig.InfoPath),
                DefaultJsonConverterSettings);
        }
        else
        {
    ...
    public static void VerifyOld()
    {
        ...
        {
            using (var file = new StreamWriter(ScuffedConfig.OldMapPath))
            {
    ...
    {
        var config = new Config { IsBackupEnabled = true, IsAutoImportEnabled = false };

        var mapfolderpath = args != null && args.Any()
            ? args[0]
            : new FileInfo(Process.GetCurrentProcess().MainModule.FileName).DirectoryName;
        // get beatmap option
        var mapFolder = new DirectoryInfo(mapfolderpath);
        var mapDataFiles = mapFolder.GetFiles("*.dat");
        ...
        Console.Write(
            $"Difficulty To Work On (overwrites everything in this difficulty) ({string.Join(",", indexoption)}):");
        var option = Convert.ToInt32(Console.ReadLine());

        Console.Write("AutoImport Map? (writes an import statement for created backup file) (y/n):");
        {
            var answer = Convert.ToChar(Console.ReadLine().ToLower());
        ...
        config.SWFilePath =
            Path.Combine(mapFolder.FullName, mapDataFiles[option].Name.Split('.')[0] + "_ScuffedWalls.sw");
        ...
        config.BackupPaths.BackupFolderPath =
            Path.Combine(mapFolder.FullName, mapDataFiles[option].Name.Split('.')[0] + "Backup");
