...
[SFunction("Wall")]
internal class Wall : ScuffedFunction
{
    private bool isNjs;
    private ICustomDataMapObject parsedthing;

    protected override void Init()
    {
        parsedthing = UnderlyingParameters.CustomDataParse(new BeatMap.Obstacle());
        isNjs = parsedthing != null && parsedthing._customData != null &&
                parsedthing._customData["_noteJumpStartBeatOffset"] != null;
    }

    protected override void Update()
    {
        var duration = GetParam("duration", 0, p => float.Parse(p));
        var lineindex = GetParam("lineindex", 0, p => int.Parse(p));
        ...
        duration = GetParam("definiteduration", duration, p =>
        {
            if (isNjs)
                return ScuffedWallsContainer.BPMAdjuster.GetDefiniteDurationBeats(p.ToFloat(),
                    parsedthing._customData["_noteJumpStartBeatOffset"].ToFloat());
            return ScuffedWallsContainer.BPMAdjuster.GetDefiniteDurationBeats(p.ToFloat());
        });
        ...
        var append = (BeatMap.Obstacle)UnderlyingParameters.CustomDataParse(new BeatMap.Obstacle());
