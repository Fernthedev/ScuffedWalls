...
public class Color
{
    ...
    {
        return new Color
            { R = array[0].ToFloat(), G = array[1].ToFloat(), B = array[2].ToFloat(), A = array[3].ToFloat() };
    }
...
public class Cube
{
    ...
    {
        if (Matrix.HasValue) Transformation = Transformation.fromMatrix(Matrix.Value);
        if (OffsetMatrix.HasValue) OffsetTransformation = Transformation.fromMatrix(OffsetMatrix.Value);
    ...
    public Cube[] InstantiateMultiples()
    {
        if (Frames != null && Frames.Any(f => f.Active != Frames.First().Active))
        {
            ...
            var framespan = new List<DoubleInt>();
    ...
    public void SetOffset()
    {
        if (Frames != null && Frames.Any() && Frames.All(f => f.Matrix.HasValue && f.Transformation != null))
            Frames = Frames.Select(frame =>
            {
                ...
                mat.Translation =
                    mat.Translation +
                    new Vector3(trans.Scale.X - 2, 0, 0); //compensate animate scale vs scale difference
    ...
    {
        var newCubes = Options.cubes.Select(c => c.Clone()).ToArray();

        var boundingbox = newCubes.Select(n => n.Matrix.Value).ToArray().GetBoundingBox().Main;

        var Offset = boundingbox.Position;
        if (Options.TransformOrigin == DeltaTransformOptions.TransformOptions.FrontBottomLeft)
            Offset = boundingbox.Position - boundingbox.Scale / 2f;
