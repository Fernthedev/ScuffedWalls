...
public class Transformation
{
    ...
    public Transformation Clone()
    {
        return new Transformation
            { Position = Position, RotationEul = RotationEul, Scale = Scale, RotationQuat = RotationQuat };
    }
...
internal static class Maths
{
    ...
    public static float[] Mult(this float[] array1, float[] array2)
    {
        if (array1.Length != array2.Length)
            throw new ArgumentOutOfRangeException("Mult() array1 length does not equal array2 length");

        for (var index = 0; index < array1.Length; index++) array1[index] = array1[index] * array2[index];
        return array1;
    }
    ...
    {
        var angleRad = Math.PI.ToFloat() / 180f * (angle - 90f);
        var x = radius * Math.Cos(angleRad).ToFloat();
        var y = radius * Math.Sin(angleRad).ToFloat();
    ...
    {
        var xOver2 = euler.X * ((float)Math.PI / 180) * 0.5f;
        var yOver2 = euler.Y * ((float)Math.PI / 180) * 0.5f;
        var zOver2 = euler.Z * ((float)Math.PI / 180) * 0.5f;
