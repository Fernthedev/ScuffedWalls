...
public class BeatMap : ICloneable
{
    ...
    [JsonIgnore] public MapStats Stats => GetStats();
    ...
    [MapStat] public List<Event> _events { get; set; } = new();
    [MapStat] public List<Note> _notes { get; set; } = new();
    [MapStat] public List<Obstacle> _obstacles { get; set; } = new();
    [MapStat] public List<Waypoint> _waypoints { get; set; } = new();

    public static BeatMap Empty => new();

    public static TreeDictionary ImportantMapCustomDataFields => new()
    {
    ...
    public static string[] NoodleExtensionsPropertyNames => new[]
    {
        _height,
        _attenuation,
        _position,
    ...
    public MapStats GetStats()
    {
        ...
        foreach (var prop in typeof(BeatMap).GetProperties()
                     .Where(p => p.GetCustomAttributes<MapStatAttribute>().Any()))
        {
            var val = prop.GetValue(this);
            if (val is IEnumerable<object> array && array.Any())
            {
                var count = array.Count();
                stats.AddStat(new KeyValuePair<string, int>(prop.Name.MakePlural(count), count));
            }
        }

        foreach (var item in _customData)
            if (item.Value is IEnumerable<object> aray && aray.Count() > 0)
                stats.AddStat(new KeyValuePair<string, int>(item.Key.MakePlural(aray.Count()), aray.Count()));
        return stats;
    }
    ...
    public void Prune()
    {
        ...
        foreach (var mapobj in _events)
            if (mapobj._customData != null)
                mapobj._customData.DeleteNullValues();
        foreach (var mapobj in _notes)
            if (mapobj._customData != null)
                mapobj._customData.DeleteNullValues();
        foreach (var mapobj in _obstacles)
            if (mapobj._customData != null)
                mapobj._customData.DeleteNullValues();
    }
    ...
    public bool needsNoodleExtensions()
    {
        ...
        if (_customData.At<IEnumerable<object>>("_customEvents").Any()) return true;
        ...
        {
            return customData.Any(p =>
                       NoodleExtensionsPropertyNames.Any(n =>
                           n == p.Key)) || //customData has one of the noodle properties listed
                   customData.At("_animation")
                       .Any(p => NoodleExtensionsPropertyNames.Any(n => n == p.Key));
        } //animation exists in custom data and has noodle params
    ...
    public bool NeedsChroma()
    {
        ...
        if (_obstacles.Any(wall => wall._customData != null && (wall._customData["_color"] != null ||
                                                                (wall._customData["_animation"] != null &&
                                                                 wall._customData["_animation._color"] != null))))
            return true;

        //do note have color or animate color
        if (_notes.Any(note => note._customData != null && (note._customData["_color"] != null ||
                                                            (note._customData["_animation"] != null &&
                                                             note._customData["_animation._color"] != null))))
            return true;
    ...
    public void Order()
    {
        var Keys = _customData.Keys.ToArray();

        foreach (var key in Keys)
            if (_customData[key] is IList<object> array && array.All(arrayitem =>
                    arrayitem is IDictionary<string, object> dict && dict.ContainsKey("_time")))
    ...
    {
        return type == MapObjectType.Note ? new Note() :
            type == MapObjectType.Obstacle ? new Obstacle() :
