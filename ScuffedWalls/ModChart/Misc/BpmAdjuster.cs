...
internal class BpmAdjuster
{
    public BpmAdjuster(float Bpm, float Njs, float NjsOffset)
    {
        this.Bpm = Bpm;
        this.Njs = Njs;
        StartBeatOffset = NjsOffset;
        HalfJumpBeats = GetJumps(StartBeatOffset, Njs, Bpm);
        SetBeatLength();
    }

    public float Bpm { get; }
    public float Njs { get; }
    public float StartBeatOffset { get; }
    ...
    /// </summary>
    public float HalfJumpBeats { get; }
    ...
    public static float GetJumps(float NjsOffset, float NJS, float BPM)
    {
        ...
        var _startNoteJumpMovementSpeed = NJS;
        var _noteJumpStartBeatOffset = NjsOffset;

        var _noteJumpMovementSpeed = _startNoteJumpMovementSpeed * BPM / BPM;
        var num = 60f / BPM;
        var num2 = _startHalfJumpDurationInBeats;
        while (_noteJumpMovementSpeed * num * num2 > _maxHalfJumpDistance) num2 /= 2;
